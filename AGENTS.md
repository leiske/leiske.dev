...
**Important**: The build script compilation step also compiles imported components from `src/` into `dist-scripts/src/`. If you modify components in `src/` and want the static HTML generation to use the updated components, you must recompile the build script: `npx tsc scripts/build-static.ts --outDir dist-scripts --module esnext --target es2023 --moduleResolution bundler --jsx react-jsx --skipLibCheck --strict`
