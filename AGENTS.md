...
- Entry point: `src/main.tsx`, Router configuration: `src/router.tsx`
- Routes in `src/routes/` directory using TanStack Router file-based routing
- Use named imports with file extensions: `import { Component } from './Component.tsx'`
- Third-party imports first, then local imports
- Type imports use `.js` extension: `import type { Post } from 'content-collections'`
...
### SEO Meta Tags with Head Function
- Routes can export a `head` function to define meta tags
- The `head` function receives `loaderData` from the route's loader
- Return an object with `meta` array containing meta tag objects
- Example:
  ```tsx
...
- Import with: `import { allPosts } from 'content-collections'` (uses path alias)
- Type import: `import type { Post } from 'content-collections'`
- Path alias configured in `tsconfig.app.json`: `"content-collections": ["./.content-collections/generated"]`
- IMPORTANT: Generated files are gitignored and auto-generated by dev server
- IMPORTANT: Do NOT import directly from `.content-collections/generated/` in source files - use the `content-collections` path alias instead to avoid infinite loop during dev server watch
...
- Posts data generated by content-collections: `import { allPosts } from 'content-collections'`
- Markdown processing in `src/utils/markdown.ts` using unified ecosystem
...
### Components
- `src/components/Markdown.tsx`: Markdown renderer with heading extraction
- `src/components/PostList.tsx`: Post list with titles/dates
- `src/components/PostContent.tsx`: Full post with navigation
- `src/routes/index.tsx`: Homepage (5 recent posts)
- `src/routes/blog.index.tsx`: Blog index (all posts)
- `src/routes/blog.$slug.tsx`: Individual blog post page

### Routes
- `src/routes/__root.tsx`: Root layout with Tailwind styles
- `src/routes/index.tsx`: Home page with recent posts
- `src/routes/blog.index.tsx`: Blog index page
- `src/routes/blog.$slug.tsx`: Blog post page with loader for data fetching
